...
assert.match(
    ...
    "LRC Maker service worker must stay registered from the project directory.",
);
assert.match(worker, /const APP_NAME = "akari-lrc-maker"/, "LRC Maker cache namespace must remain explicit.");
assert.match(
    worker,
    /cacheName\.startsWith\(APP_NAME\) && cacheName !== CACHENAME/,
    "LRC Maker must delete only its own obsolete caches.",
);
