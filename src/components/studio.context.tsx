...
export const StudioProvider: React.FC<React.PropsWithChildren> = ({ children }) => {
    ...
    const [status, setStatus] = useState<StudioState["status"]>(initialLaunch ? "loading" : "standalone");
    ...
    const reload = useCallback(async () => {
        ...
        try {
            const response = await fetch(
                adminUrl(`/api/studio/tracks/${encodeURIComponent(initialLaunch.trackId)}/lyrics/context`),
                { credentials: "include", headers: { Accept: "application/json" } },
            );
            const next = await readJson<LyricsContextPayload>(response);
            if (next.context !== "lyrics-studio-v1" || next.trackId !== initialLaunch.trackId) {
    ...
    const saveLyrics = useCallback(async (lyrics: string) => {
        if (!initialLaunch || !context) throw new Error("Contexte Lyrics indisponible.");
        setStatus("saving");
        setMessage("");
        const common = {
            trackId: initialLaunch.trackId,
            lyrics,
        ...
        try {
            const post = async (suffix: string, intent: string): Promise<SavePayload> => {
                const response = await fetch(
                    adminUrl(`/api/studio/tracks/${encodeURIComponent(initialLaunch.trackId)}/lyrics/sync/${suffix}`),
                    {
                        method: "POST",
                        credentials: "include",
            ...
            if (refreshed.trackId !== initialLaunch.trackId || refreshed.lyrics.text !== lyrics) {
            ...
            window.opener?.postMessage(
                {
                    ...
                    trackId: initialLaunch.trackId,
                    updatedAt: refreshed.lyrics.updatedAt,
                },
                location.origin,
            );
        } catch (error) {
    ...
    const returnToStudio = useCallback(() => {
        if (initialLaunch?.returnPath) location.assign(initialLaunch.returnPath);
        else window.close();
    }, []);
    ...
    const value = useMemo<StudioState>(() => ({
        ...
        saveLyrics,
        returnToStudio,
    }), [context, message, reload, returnToStudio, saveLyrics, status]);
